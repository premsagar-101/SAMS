...
export interface IPeriod extends Document {
  ...
  isWithinGracePeriod(graceMinutes: number): boolean;
}
...
// Instance methods
periodSchema.methods.isCurrent = function(): boolean {
  const now = new Date();
  return this.startTime <= now && this.endTime >= now;
};

periodSchema.methods.isPast = function(): boolean {
  return this.endTime < new Date();
};

periodSchema.methods.isFuture = function(): boolean {
  return this.startTime > new Date();
};

periodSchema.methods.getDurationMinutes = function(): number {
  return Math.floor((this.endTime.getTime() - this.startTime.getTime()) / (1000 * 60));
};

periodSchema.methods.canGenerateQR = function(): boolean {
  const now = new Date();
...
periodSchema.methods.isWithinGracePeriod = function(graceMinutes: number): boolean {
  const now = new Date();
...
periodSchema.statics.findByDate = function(date: Date, teacherId?: mongoose.Types.ObjectId) {
  const query: any = {
    date: {
...
periodSchema.statics.findCurrent = function(teacherId?: mongoose.Types.ObjectId) {
  const now = new Date();
  const query: any = {
    startTime: { $lte: now },
    endTime: { $gte: now },
...
periodSchema.statics.findBySemester = function(semesterId: mongoose.Types.ObjectId) {
...
periodSchema.statics.generateDailySchedule = function(date: Date) {
  const startOfDay = new Date(date);
  startOfDay.setHours(0, 0, 0, 0);
  const endOfDay = new Date(date);
  endOfDay.setHours(23, 59, 59, 999);
