...
qrSessionSchema.virtual('timeRemaining', {
  ref: 'Period',
  localField: 'period',
  foreignField: '_id',
  justOne: true
...
// Instance methods
qrSessionSchema.methods.isExpired = function(): boolean {
  return new Date() > this.expiresAt;
};

qrSessionSchema.methods.getTimeRemaining = function(): number {
  const now = new Date().getTime();
  const expiry = this.expiresAt.getTime();
  return Math.max(0, expiry - now);
};

qrSessionSchema.methods.extendExpiry = function(minutes: number): void {
...
// Static methods
qrSessionSchema.statics.findByPeriod = function(periodId: mongoose.Types.ObjectId) {
...
qrSessionSchema.statics.findActiveByTeacher = function(teacherId: mongoose.Types.ObjectId) {
  return this.find({
    isActive: true,
    expiresAt: { $gt: new Date() }
  })
...
qrSessionSchema.statics.createOrUpdate = async function(periodId: mongoose.Types.ObjectId) {
...
qrSessionSchema.statics.getStatsByPeriod = function(periodId: mongoose.Types.ObjectId) {
