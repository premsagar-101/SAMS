...
export interface ISemester extends Document {
  ...
  isOverlapping(startDate: Date, endDate: Date): boolean;
}
...
// Instance methods
semesterSchema.methods.getDurationDays = function(): number {
  const timeDiff = this.endDate.getTime() - this.startDate.getTime();
  return Math.ceil(timeDiff / (1000 * 3600 * 24));
};

semesterSchema.methods.isOverlapping = function(startDate: Date, endDate: Date): boolean {
  return (
...
// Static methods
semesterSchema.statics.findCurrent = function(programId?: mongoose.Types.ObjectId) {
  const query: any = { isCurrent: true, isActive: true };
  if (programId) {
...
semesterSchema.statics.findByProgram = function(programId: mongoose.Types.ObjectId) {
  return this.find({ program: programId, isActive: true })
    .sort({ startDate: -1 })
    .populate('program', 'name code');
};

semesterSchema.statics.findByAcademicYear = function(academicYear: string) {
