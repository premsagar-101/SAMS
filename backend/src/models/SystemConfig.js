...
// Instance methods
systemConfigSchema.methods.getTypedValue = function() {
...
systemConfigSchema.methods.updateValue = async function(newValue) {
  this.value = newValue;
  await this.save();
};
...
// Static methods
systemConfigSchema.statics.findByKey = function(key) {
  return this.findOne({ key: key, isActive: true });
};

systemConfigSchema.statics.findByCategory = function(category) {
  return this.find({ category: category, isActive: true })
    .sort({ key: 1 });
};

systemConfigSchema.statics.getValue = async function(key, defaultValue) {
...
systemConfigSchema.statics.getAllConfigs = function(category) {
...
systemConfigSchema.statics.getCategoryFromKey = function(key) {
