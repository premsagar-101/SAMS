...
// Instance methods
timetableSchema.methods.getTimeRange = function() {
...
timetableSchema.methods.isTimeConflict = function(otherTimetable) {
...
timetableSchema.methods.getDurationMinutes = function() {
  const start = this.stringToMinutes(this.startTime);
  const end = this.stringToMinutes(this.endTime);
  return end - start;
};

timetableSchema.methods.stringToMinutes = function(timeString) {
...
timetableSchema.statics.findBySemester = function(semesterId) {
...
timetableSchema.statics.findByRoom = function(
  ...
  const query = { room: room.trim(), isActive: true };
  if (semesterId) {
    query.semester = semesterId;
  }
...
timetableSchema.statics.findCurrent = function(teacherId) {
  const now = new Date();
